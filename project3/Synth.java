...
/**
 * The Synthesizer module. In setup(), you create the various modules, in order,
 * and add them to the synth. Call setOuput() to specify which module has its
 * output handed to audio. Then Synth calls updateValue() on each in turn and
 * generates sound.
 */
...
public class Synth {
	...
	/**
	 * Size of the Java SourceDataLine audio buffer. Larger and we have more
	 * latency. Smaller and the system can't handle it. It appears &geq 1024 is
	 * required on the Mac for 44100 KHz
	 */
	 ...
	 * 
	 * Each Sound has its own random number generator. You can get a new, more or
	 * less statistically independent generator from this method.
	 */
	...
	public void setup() {
		...
		Options algorithmOptions = new Options("Algorithms", new String[] { "Parallel", "Serial", "Branch", "Merge" },
				1);
		...
		Options operator1RelativeFrequencyOptions = new Options("Operator 1 Relative Frequency", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		Options operator1PhaseAmplitudeOptions = new Options("Operator 1 Phase Amplitude", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		...
		Options operator2RelativeFrequencyOptions = new Options("Operator 2 Relative Frequency", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		Options operator2PhaseAmplitudeOptions = new Options("Operator 2 Phase Amplitude", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		...
		Options operator3RelativeFrequencyOptions = new Options("Operator3 Relative Frequency", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		Options operator3PhaseAmplitudeOptions = new Options("Operator 3 Phase Amplitude", new String[] { "0.25",
				"0.50", "0.75", "1.00", "1.25", "1.50", "1.75", "2.00", "2.25", "2.50", "2.75", "3.00" }, 3);
		...
		ADSR operator1ADSR = new ADSR();
		operator1ADSR.gate = gate;
		ADSR operator2ADSR = new ADSR();
		operator2ADSR.gate = gate;
		ADSR operator3ADSR = new ADSR();
		operator3ADSR.gate = gate;
		ADSR masterADSR = new ADSR();
		masterADSR.gate = gate;
		// add them to be updated
		modules.add(operator1ADSR);
		modules.add(operator2ADSR);
		modules.add(operator3ADSR);
		modules.add(masterADSR);
		operator1ADSR.setAttackTime(operator1AttackDial.getModule());
		operator1ADSR.setDecayTime(operator1DecayDial.getModule());
		operator1ADSR.setSustainLevel(operator1SustainDial.getModule());
		operator1ADSR.setReleaseTime(operator1ReleaseDial.getModule());
		operator2ADSR.setAttackTime(operator2AttackDial.getModule());
		operator2ADSR.setDecayTime(operator2DecayDial.getModule());
		operator2ADSR.setSustainLevel(operator2SustainDial.getModule());
		operator2ADSR.setReleaseTime(operator2ReleaseDial.getModule());
		operator3ADSR.setAttackTime(operator3AttackDial.getModule());
		operator3ADSR.setDecayTime(operator3DecayDial.getModule());
		operator3ADSR.setSustainLevel(operator3SustainDial.getModule());
		operator3ADSR.setReleaseTime(operator3ReleaseDial.getModule());

		masterADSR.setAttackTime(masterADSRAttackDial.getModule());
		masterADSR.setDecayTime(masterADSRDecayDial.getModule());
		masterADSR.setSustainLevel(masterADSRSustainDial.getModule());
		masterADSR.setReleaseTime(masterADSRReleaseDial.getModule());
		PM h = new PM();
