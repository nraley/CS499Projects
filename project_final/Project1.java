    ...
    public void setup() 
    	{
			...
			//Building the Sequencer -> make a box -> make drop down menus -> dials ->??? -> profit
			Box sBox = new Box(BoxLayout.Y_AXIS);
			sBox.setBorder(BorderFactory.createTitledBorder("Sequencer"));
			outer.add(sBox);
			//making the space for the row of pitchboxes 
			Box pitchBox = new Box(BoxLayout.X_AXIS);
			sBox.add(pitchBox);
			String[] NOTES = new String[]{"A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#","G","G#"};
			ArrayList<Options> pitchSteps = new ArrayList<Options>();
			for(int i = 0; i < 8; i++){
				pitchSteps.add(new Options("", NOTES, 0));
				pitchBox.add(pitchSteps.get(i));
			}
			//making space for the row of velocityboxes
			Box velBox = new Box(BoxLayout.X_AXIS);
			sBox.add(velBox);
			// create the 8 dials for velocity
			ArrayList<Dial> velSteps = new ArrayList<Dial>();
				for(int i = 0; i < 8; i++){
				velSteps.add(new Dial(1.0));
				velBox.add(velSteps.get(i).getLabelledDial("Velocity"));
				}
